...
<script>
import {getHistoryMessage, getMessageUser, wkSendStudent, wkBindTeacher, wkBindStudent} from "@/api/user/index";
  ...
  methods: {
    ...
    async bindWinBar(play = {}) {
      ...
      if (type === "winBar") {
        ...
        this.taleList = [];
        const res = await getHistoryMessage({student_id: id});
        if (res.code !== 200) {
    ...
    async sendMsg(msg) {
      let res = await wkSendStudent({user_id: this.winBarConfig.active, message: msg});
    },
  },
